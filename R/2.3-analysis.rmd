...
png(width = 3 * 600, height = 2 * 300, filename = fname)
...
p3 <- ggplot(data = long_nt, aes(x = year, y = tss, color = model)) +
    geom_point() +
    geom_line() +
    coord_cartesian(xlim = c(2002, max(years)), ylim = c(0, 1)) +
    ggtitle("tss for predicting the following year (native data model)")
p4 <- ggplot(data = long_t, aes(x = year, y = tss, color = model)) +
    geom_point() +
    geom_line() +
    coord_cartesian(xlim = c(2002, max(years)), ylim = c(0, 1)) +
    ggtitle("tss for predicting the trained dataset")
p5 <- ggplot(data = long_fy, aes(x = year, y = tss, color = model)) +
    geom_point() +
    geom_line() +
    coord_cartesian(xlim = c(2002, max(years)), ylim = c(0, 1)) +
    ggtitle("tss for predicting year (invasive data model)")
p6 <- ggplot(data = long_22, aes(x = year, y = tss, color = model)) +
    geom_point() +
    geom_line() +
    coord_cartesian(xlim = c(2002, max(years)), ylim = c(0, 1)) +
...
png(width = 800, height = 600, filename = fname)
ggplot(crop(countries, ext(lc_ref))) +
...
image_write(
    image = img_animated,
    path = "R/plots/niche_comp/eu_years_niche.gif"
)
```

```{r, plot niche dynamic results}
library(ggplot2)
dyn <- as.data.frame(readRDS("R/data/modelling/niche_y_dynamic.rds"))
years <- 2002:2022

ggplot(dyn, aes(x = years)) +
    geom_point(aes(y = expansion), colour = "red", show.legend = TRUE) +
    geom_line(aes(y = expansion), colour = "red") +
    geom_point(aes(y = stability), colour = "blue", show.legend = TRUE) +
    geom_line(aes(y = stability), colour = "blue") +
    geom_point(aes(y = unfilling), colour = "green", show.legend = TRUE) +
    geom_line(aes(y = unfilling), colour = "green")
```

...
# plot
p1 <- ggplot() +
    geom_spatvector(data = pts, size = 1) +
    theme_pubr() +
...
p <- ggarrange(p1, p2, nrow = 1, ncol = 2, labels = c("A", "B")) + bgcolor("white")
ggsave(p, width = 10, height = 4, filename = "R/figures/ext-subdiv.png")
```
...
p <- ggplot(data = long_dyn, aes(x = year, y = value, color = index, shape = index)) +
    geom_point() +
    geom_line() +
    ...
    theme_pubr()
ggsave(p, width = 8, height = 5, filename = "R/figures/niche-dyn-ol-res.png")
```
...
overlaps <- readRDS("R/data/modelling/niche_y_overlap.rds")
overlaps <- head(overlaps, nrow(tss_res))

...
pa <- subset(pa, Area == "eu" & Year == 2015)
...
saveRDS(tss_fy, file = "R/data/modelling/mod_tss_res_nat.rds")
...
# plot
fname <- paste0("R/plots/model_performance_results_subdiv.png")
...
p3 <- ggplot(data = long_nt, aes(x = year, y = tss, color = model)) +
    ...
    geom_line() +
    coord_cartesian(xlim = c(2002, max(years)), ylim = c(0, 1)) +
    ggtitle("tss for predicting year (native data model)")
...
pa_vals <- subset(pa_vals, Area == "eu")

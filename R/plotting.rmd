...
png(width = 600, height = 600, file = "R/plots/abs_gen_subdiv_bench_dummy.png")
barplot(mat,
...
```{r, benchmark abs_gen with cleaned data (runtime 42 mins)}
rtime <- Sys.time()
set.seed(4326)
source("R/0.0-functions.r", encoding = "UTF-8")
...
barplot(mat,
    col = rainbow(3), beside = TRUE,
    main = "Runtime comparison of different subset resolutions (min) with occurrence subsets"
)
legend("topleft", legend = rownames(mat), fill = rainbow(3))
dev.off()
```
...
for (s in seq_len(nrow(subexts))) {
    ext_s <- ext(subexts[s, ])
    lines(vect(ext_s, crs = "epsg:4326"), col = colors[s])
}
```
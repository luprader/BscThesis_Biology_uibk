...
occs_eu <- subset(occs, Area == "EU")
occs_as <- subset(occs, Area == "AS")

# get occurences per year as df
tot_freq <- as.data.frame(table(occs$Year))
eu_freq <- as.data.frame(table(occs_eu$Year))
as_freq <- as.data.frame(table(occs_as$Year))
...
plot <- ggplot(frequencies, aes(x = Year, y = Freq)) +
    ...
    scale_y_continuous(trans = "log10") + scale_x_discrete( breaks = yearbreaks)
plot

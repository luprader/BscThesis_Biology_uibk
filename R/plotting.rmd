...
for (i in seq_len(nrow(exts))) {
    ...
    t_endcounts <- c(pcnt / 10, pcnt / 2, pcnt / 100)
    for (j in seq_along(t_endcounts)) {
        tot_time <- Sys.time()
        e_cnt <- t_endcounts[j]
        subexts <- lp_subdiv_pts(t_pres, e_cnt, t_ext)
        all_subexts <- rbind(all_subexts, subexts)
        pa <- data.frame() # initialize pa df
        prog <- 0
...
saveRDS(results, file = "gen_benchmarkresults.rds")
print(Sys.time() - rtime) # total runtime of cell
```

```{r, barplot with benchmark results}
results <- readRDS("R/data/plotting/gen_benchmarkresults.rds")
...
png(width = 600, height = 600, file = "R/plots/abs_gen_subdiv_bench.png")
barplot(mat,
    col = rainbow(3), beside = TRUE,
    main = "Runtime comparison of different subset resolutions (min)"
)

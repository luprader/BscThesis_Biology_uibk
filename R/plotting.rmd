...
d_rast <- rasterize(rd_crop, rast(rd_crop, ncols = 1000, nrows = 1000), "Presence")
# flag wrong absences
...
```{r}
clean <- readRDS("R/data/occurence_data/axyridis_clean.rds")
clean <- subset(clean, Year >= 2002)
for (i in 2002:2020) {
    cat(i, nrow(subset(clean, Year <= i)), "\n")
}
```